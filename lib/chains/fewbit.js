...
module.exports = {
  ...
  // fewbit-network/Core-Wallet `src/evo/providertx.h`). Dash's default
  // is `2`, which FewBit's `CheckProRegTx` rejects.
  payloadVersions: {
    proRegTx: 1,
  },

  // FewBit's `CheckProRegTx` rejects any payload with `type !== 0`
  // (`MASTERNODE_TYPE_BASIC`). Enabling this flag makes the lib fail
  // fast in `validate()` instead of producing an invalid wire payload.
