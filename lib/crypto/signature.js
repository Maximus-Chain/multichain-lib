...
Signature.fromCompact = function (buf) {
  ...
  $.checkArgument(
    b2.length === 32,
    new Error('r must be 32 bytes (got ' + b2.length + ')')
  );
  $.checkArgument(
    b3.length === 32,
    new Error('s must be 32 bytes (got ' + b3.length + ')')
  );
...
Signature.prototype.toCompact = function (i, compressed) {
  ...
  // BN.toBuffer({size: 32}) is documented but does not always left-pad to
  // the requested size in all bn.js versions; for some ECDSA-K values it
  // returns a shorter buffer, which causes fromCompact to throw "r/s must
  // be 32 bytes". To make Signature.toCompact deterministic and stable
  // across Node and bn.js versions, we explicitly pad r and s to exactly
  // 32 bytes here.
  function pad32(buf) {
  ...
  var b2 = pad32(this.r.toBuffer({ size: 32 }));
  var b3 = pad32(this.s.toBuffer({ size: 32 }));
