...
var hdPrivateKeyClassCache = new WeakMap();

/**
 * Creates an HDPrivateKey class bound to the given `Network` instance. See
 * `lib/address.js`'s `createAddressClass` for the isolation and memoization
 * rationale.
 *
 * @param {Object} Network - a `lib/networks.js` `createNetworks()` instance
 * @returns {HDPrivateKey}
 */
function createHDPrivateKeyClass(Network) {
  if (hdPrivateKeyClassCache.has(Network)) {
    return hdPrivateKeyClassCache.get(Network);
  }

  function getPrivateKey() {
    return require('./privatekey').createPrivateKeyClass(Network);
  }

  function getHDPublicKey() {
    return require('./hdpublickey').createHDPublicKeyClass(Network);
  }
  ...
  HDPrivateKey._getDerivationIndexes = function (path) {
    ...
    var indexes = steps.slice(1).map(function (step) {
      ...
      // BIP32 Compatibility Mode
        ...
        var indexValue = HDPrivateKey._IndexArgToNumber(step, {
  ...
  HDPrivateKey.prototype._deriveWithNumber = function (
    ...
    if (!getPrivateKey().isValid(privateKey)) {
  ...
  HDPrivateKey.getSerializedError = function (data, network) {
    ...
    if (
      ![HDPrivateKey.DataLength, HDPrivateKey.DataSize256bit].includes(
        data.length
      )
    ) {
  ...
  HDPrivateKey._validateNetwork = function (data, networkArg) {
    ...
    if (
      data.length === HDPrivateKey.DataLength &&
      versionInt !== network.xprivkey
    )
      return new errors.InvalidNetwork(version);

    if (
      data.length === HDPrivateKey.DataSize256bit &&
      versionInt !== network.xprivkey256bit
    )
      return new errors.InvalidNetwork(version);
  ...
  HDPrivateKey._isIndex256bit = function (index) {
    if (
      BufferUtil.isBuffer(index) &&
      index.length === HDPrivateKey.ChildIndexSize256bit
    ) {
      return true;
    } else if (_.isString(index) && index.slice(0, 2) === '0x') {
      var bnIndex = new BN(index.slice(2, index.length), 16);
      assert(
        bnIndex.lt(HDPrivateKey.MaxIndex256bit),
        'Index is too large, should be < 2^256'
      );
      return bnIndex.gte(HDPrivateKey.MaxIndex32bit) ? true : false;
    } else if (BN.isBN(index)) {
      assert(
        index.gte(HDPrivateKey.MaxIndex32bit),
        "Index is too small, 'hex' index should be >= 2^32"
      );
      assert(
        index.lt(HDPrivateKey.MaxIndex256bit),
        'Index is too large, should be < 2^256'
      );
      return true;
    } else {
  ...
  HDPrivateKey._index256bitToBuffer = function (index) {
    if (BufferUtil.isBuffer(index)) {
      ...
      var bnIndex = new BN(index.slice(2, index.length), 16);
      assert(
        bnIndex.gte(HDPrivateKey.MaxIndex32bit),
        "Index is too small, 'hex' index should be >= 2^32"
      );
      assert(
        bnIndex.lt(HDPrivateKey.MaxIndex256bit),
  ...
  HDPrivateKey._index32bitToBuffer = function (index) {
    if (BufferUtil.isBuffer(index)) {
      assert(
        index.length === HDPrivateKey.ChildIndexSize,
  ...
  HDPrivateKey.prototype._calcHDPublicKey = function () {
    if (!this._hdPublicKey) {
      var HDPublicKey = getHDPublicKey();
  ...
  HDPrivateKey.prototype._buildFromBuffers = function (arg) {
    ...
    var privateKey = new (getPrivateKey())(BN.fromBuffer(arg.privateKey), network);
  ...
  HDPrivateKey.MaxIndex256bit = new BN(
  ...
  HDPrivateKey.HardenedEnd =
    HDPrivateKey.HardenedStart + HDPrivateKey.HardenedSize;
  HDPrivateKey.ChildIndexStart256bit = HDPrivateKey.HardenedEnd;
  HDPrivateKey.ChildIndexEnd256bit =
    HDPrivateKey.ChildIndexStart256bit + HDPrivateKey.ChildIndexSize256bit;
  HDPrivateKey.ChainCodeStart256bit = HDPrivateKey.ChildIndexEnd256bit;
  HDPrivateKey.ChainCodeEnd256bit =
    HDPrivateKey.ChainCodeStart256bit + HDPrivateKey.ChainCodeSize;
  HDPrivateKey.PrivateKeyStart256bit = HDPrivateKey.ChainCodeEnd256bit + 1;
  HDPrivateKey.PrivateKeyEnd256bit =
    HDPrivateKey.PrivateKeyStart256bit + HDPrivateKey.PrivateKeySize;
  HDPrivateKey.ChecksumStart256bit = HDPrivateKey.PrivateKeyEnd256bit;
  HDPrivateKey.ChecksumEnd256bit =
    HDPrivateKey.ChecksumStart256bit + HDPrivateKey.CheckSumSize;

  assert(HDPrivateKey.ChecksumEnd === HDPrivateKey.SerializedByteSize);
  assert(
    HDPrivateKey.ChecksumEnd256bit === HDPrivateKey.SerializedByteSize256bit
  );

  hdPrivateKeyClassCache.set(Network, HDPrivateKey);

  return HDPrivateKey;
}

module.exports = { createHDPrivateKeyClass: createHDPrivateKeyClass };
