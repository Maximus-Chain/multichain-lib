...
var hdPublicKeyClassCache = new WeakMap();

/**
 * Creates an HDPublicKey class bound to the given `Network` instance. See
 ...
 * @returns {HDPublicKey}
 */
function createHDPublicKeyClass(Network) {
  if (hdPublicKeyClassCache.has(Network)) {
    return hdPublicKeyClassCache.get(Network);
  }

  function getHDPrivateKey() {
    return require('./hdprivatekey').createHDPrivateKeyClass(Network);
  }

  function getPublicKey() {
    return require('./publickey').createPublicKeyClass(Network);
  }
      ...
      if (_.isString(arg) || BufferUtil.isBuffer(arg)) {
        ...
        if (!error) {
          ...
          if (error instanceof hdErrors.ArgumentIsPrivateExtended) {
            return new (getHDPrivateKey())(arg).hdPublicKey;
          }
          throw error;
        }
      } else {
        if (_.isObject(arg)) {
          if (arg instanceof getHDPrivateKey()) {
  ...
  HDPublicKey.isValidPath = function (arg) {
    if (_.isString(arg)) {
      var indexes = getHDPrivateKey()._getDerivationIndexes(arg);
      return (
        indexes !== null &&
        _.every(
    ...
    if (BN.isBN(arg)) {
      return (
        arg.gte(HDPublicKey.MaxIndex32bit) && arg.lt(HDPublicKey.MaxIndex256bit)
      );
    }
  ...
  HDPublicKey.prototype.deriveChild = function (arg, hardened) {
    var HDPrivateKey = getHDPrivateKey();
    // DIP14 256-bit Mode
  ...
  HDPublicKey.prototype._deriveWithNumber = function (index, hardened) {
    ...
    var publicKey;
    try {
      publicKey = getPublicKey().fromPoint(
  ...
  HDPublicKey.prototype._deriveFromString = function (path) {
    ...
    var indexes = getHDPrivateKey()._getDerivationIndexes(path);
  ...
  HDPublicKey.prototype._buildFromPrivate = function (arg) {
    ...
    var versionKey = getHDPrivateKey()._isIndex256bit(arg._buffers.childIndex)
      ? 'xpubkey256bit'
      : 'xpubkey';
  ...
  HDPublicKey.prototype._buildFromBuffers = function (arg) {
    ...
    var publicKey = new (getPublicKey())(arg.publicKey, { network: network });
  ...
  HDPublicKey.MaxIndex32bit = new BN(getHDPrivateKey().MaxIndex);
  ...
  hdPublicKeyClassCache.set(Network, HDPublicKey);

  return HDPublicKey;
}

module.exports = { createHDPublicKeyClass: createHDPublicKeyClass };
