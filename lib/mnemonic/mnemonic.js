...
var mnemonicClassCache = new WeakMap();

/**
 * Creates a Mnemonic class bound to the given `Networks` instance (needed
 * for `toHDPrivateKey()`'s `HDPrivateKey`).
 *
 * @param {Object} Networks - a `lib/networks.js` `createNetworks()` instance
 * @returns {Mnemonic}
 */
function createMnemonicClass(Networks) {
  if (mnemonicClassCache.has(Networks)) {
    return mnemonicClassCache.get(Networks);
  }

  var HDPrivateKey = require('../hdprivatekey').createHDPrivateKeyClass(
    Networks
  );
  ...
  mnemonicClassCache.set(Networks, Mnemonic);

  return Mnemonic;
}

module.exports = { createMnemonicClass: createMnemonicClass };
