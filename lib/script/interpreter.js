...
var interpreterClassCache = new WeakMap();

/**
 * Creates the Interpreter class bound to the given `Networks` instance
 * (needed for its `Script`, `PublicKey`, and the default empty `Transaction`
 * used by `verify()` when no transaction context is given). Interpreter's
 * own logic is fully network-independent — a stack machine executing
 * already-built scripts.
 *
 * @param {Object} Networks - a `lib/networks.js` `createNetworks()` instance
 * @returns {Interpreter}
 */
function createInterpreterClass(Networks) {
  if (interpreterClassCache.has(Networks)) {
    return interpreterClassCache.get(Networks);
  }

  var Script = require('./script').createScriptClass(Networks);
...
Interpreter.prototype.step = function () {
  ...
  interpreterClassCache.set(Networks, Interpreter);

  return Interpreter;
}

module.exports = { createInterpreterClass: createInterpreterClass };
