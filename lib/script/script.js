...
var scriptClassCache = new WeakMap();

/**
 * Creates a Script class bound to the given `Networks` instance. See
 * `lib/address.js`'s `createAddressClass` for the isolation and memoization
 * rationale; this factory follows the same pattern, resolving `Address` and
 * `PublicKey` lazily (and by the same `Networks` identity) to avoid the
 * circular require between `Script` and `Address`.
 *
 * @param {Object} Networks - a `lib/networks.js` `createNetworks()` instance
 * @returns {Script}
 */
function createScriptClass(Networks) {
  if (scriptClassCache.has(Networks)) {
    return scriptClassCache.get(Networks);
  }
  ...
  var Script = function Script(from) {
    ...
    } else if (from instanceof getAddress()) {
  ...
  Script.prototype.isPublicKeyOut = function () {
    if (
      ...
      if (isVersion) {
        return getPublicKey().isValid(pubkeyBuf);
      }
    }
    return false;
  };
  ...
  Script.buildMultisigOut = function (publicKeys, threshold, opts) {
    ...
    publicKeys = _.map(publicKeys, getPublicKey());
  ...
  Script.buildPublicKeyOut = function (pubkey) {
    $.checkArgument(pubkey instanceof getPublicKey());
    var s = new Script();
  ...
  scriptClassCache.set(Networks, Script);

  return Script;
}

module.exports = { createScriptClass: createScriptClass };
