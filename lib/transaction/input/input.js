...
var inputClassCache = new WeakMap();

/**
 * Creates the base Input class bound to the given `Networks` instance (used
 * for its `Script` and `Sighash`).
 *
 * @param {Object} Networks - a `lib/networks.js` `createNetworks()` instance
 * @returns {Input}
 */
function createInputClass(Networks) {
  if (inputClassCache.has(Networks)) {
    return inputClassCache.get(Networks);
  }
  ...
  inputClassCache.set(Networks, Input);

  return Input;
}

module.exports = { createInputClass: createInputClass };
