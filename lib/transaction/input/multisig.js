...
var multiSigInputClassCache = new WeakMap();

/**
 * Creates the MultiSigInput class bound to the given `Networks` instance.
 *
 * @param {Object} Networks - a `lib/networks.js` `createNetworks()` instance
 * @returns {MultiSigInput}
 */
function createMultiSigInputClass(Networks) {
  if (multiSigInputClassCache.has(Networks)) {
    return multiSigInputClassCache.get(Networks);
  }
  ...
  MultiSigInput.prototype.isValidSignature = function (
    transaction,
    signature
  ) {
  ...
  multiSigInputClassCache.set(Networks, MultiSigInput);

  return MultiSigInput;
}

module.exports = { createMultiSigInputClass: createMultiSigInputClass };
