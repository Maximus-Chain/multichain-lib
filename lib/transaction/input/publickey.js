...
var publicKeyInputClassCache = new WeakMap();

/**
 * Creates the PublicKeyInput class bound to the given `Networks` instance.
 *
 * @param {Object} Networks - a `lib/networks.js` `createNetworks()` instance
 * @returns {PublicKeyInput}
 */
function createPublicKeyInputClass(Networks) {
  if (publicKeyInputClassCache.has(Networks)) {
    return publicKeyInputClassCache.get(Networks);
  }
  ...
  publicKeyInputClassCache.set(Networks, PublicKeyInput);

  return PublicKeyInput;
}

module.exports = { createPublicKeyInputClass: createPublicKeyInputClass };
