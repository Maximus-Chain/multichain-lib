...
const outputClassCache = new WeakMap();

/**
 * Creates an Output class bound to the given `Networks` instance (needed for
 * its `Script` — structural only, but there is no default `Script` class
 * anymore).
 *
 * @param {Object} Networks - a `lib/networks.js` `createNetworks()` instance
 * @returns {Output}
 */
function createOutputClass(Networks) {
  if (outputClassCache.has(Networks)) {
    return outputClassCache.get(Networks);
  }
  ...
  outputClassCache.set(Networks, Output);

  return Output;
}

module.exports = { createOutputClass };
