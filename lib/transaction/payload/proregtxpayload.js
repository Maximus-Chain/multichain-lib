 ...
 *   default `version` and to enable `type === 0` strict validation when the
 *   chain's Core rejects other types (e.g. FewBit).
 * @returns {ProRegTxPayload}
 */
function createProRegTxPayloadClass(Networks, config) {
  var cacheKey = config ? [Networks, config] : Networks;
  if (proRegTxPayloadClassCache.has(cacheKey)) {
    return proRegTxPayloadClassCache.get(cacheKey);
  }
  ...
  proRegTxPayloadClassCache.set(cacheKey, ProRegTxPayload);
