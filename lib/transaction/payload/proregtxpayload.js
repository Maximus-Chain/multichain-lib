...
var DEFAULT_PAYLOAD_VERSION = 2;
var MASTERNODE_TYPE_BASIC = 0;

var proRegTxPayloadClassCache = new WeakMap();

function activeNetworkSupportsIPv6(Networks) {
  try {
    var active = Networks.getActive();
    return Boolean(active && active.supportsIPv6);
  } catch (e) {
    return false;
  }
}
...
/**
 * Resolves the default `version` for `ProRegTxPayload` on a given chain.
 *
 * Defaults to Dash's value (`2`); FewBit and other Dash forks that only
 * ship the v1 wire format declare `payloadVersions.proRegTx` in their
 * `ChainConfig` to override this (FewBit = `1`).
 *
 * @param {Object} [config] - the chain's `ChainConfig`
 * @returns {number}
 */
function resolveProRegTxVersion(config) {
  if (
    config &&
    config.payloadVersions &&
    typeof config.payloadVersions.proRegTx === 'number'
  ) {
    return config.payloadVersions.proRegTx;
  }
  return DEFAULT_PAYLOAD_VERSION;
}

/**
 * Creates a ProRegTxPayload class bound to the given `Networks` instance
 * (used for `scriptPayout`'s address (de)serialization in the constructor
 * and `toJSON()`).
 *
 * @param {Object} Networks - a `lib/networks.js` `createNetworks()` instance
 * @param {Object} [config] - the chain's `ChainConfig`. Used to override the
 *   default `version` and to enable `type === 0` and `mode === 0` strict
 *   validation when the chain's Core rejects other values (e.g. FewBit,
 *   FILOPOW).
 * @returns {ProRegTxPayload}
 */
function createProRegTxPayloadClass(Networks, config) {
  var byConfig = proRegTxPayloadClassCache.get(Networks);
  if (!byConfig) {
    byConfig = new Map();
    proRegTxPayloadClassCache.set(Networks, byConfig);
  }
  if (byConfig.has(config)) {
    return byConfig.get(config);
  }

  var defaultVersion = resolveProRegTxVersion(config);
  var enforceBasicType = Boolean(
    config && config.enforceMasternodeTypeBasic
  );
  var enforceBasicMode = Boolean(
    config && config.enforceMasternodeModeBasic
  );

  var Script = require('../../script/script').createScriptClass(Networks);
  var PublicKey = require('../../publickey').createPublicKeyClass(Networks);
  ...
  ProRegTxPayload.fromBuffer = function fromBuffer(rawPayload) {
    ...
    // PLATFORM FIELDS DISABLED IN THIS FORK
  ...
  ProRegTxPayload.prototype.validate = function () {
    ...
    if (this.type !== undefined && this.type !== null) {
      ...
      if (enforceBasicType && this.type !== MASTERNODE_TYPE_BASIC) {
        throw new errors.InvalidArgument(
          'type',
          'Expected type to be 0 (MASTERNODE_TYPE_BASIC) on this chain'
        );
      }
    }
    if (this.mode !== undefined && this.mode !== null) {
      Preconditions.checkArgument(
        utils.isUnsignedInteger(this.mode),
        'Expect mode to be an unsigned integer'
      );
      if (enforceBasicMode && this.mode !== MASTERNODE_TYPE_BASIC) {
        throw new errors.InvalidArgument(
          'mode',
          'Expected mode to be 0 on this chain'
        );
      }
    }
    ...
    if (!ipUtil.isZeroAddress(this.service)) {
      var isIPv4Service = ipUtil.isIPV4(this.service);
      var isIPv6Service = ipUtil.isIPV6(this.service);
      var chainSupportsIPv6 = activeNetworkSupportsIPv6(Networks);
      Preconditions.checkArgument(
        (isIPv4Service && !isIPv6Service) ||
          (isIPv6Service && chainSupportsIPv6),
    ...
    // PLATFORM FIELDS VALIDATION DISABLED IN THIS FORK
  ...
  ProRegTxPayload.prototype.toJSON = function toJSON(options) {
    ...
    // PLATFORM FIELDS JSON SERIALIZATION DISABLED IN THIS FORK
    // if (this.version >= 2 && this.type === MASTERNODE_TYPE_HP) {
  ...
  ProRegTxPayload.prototype.toBuffer = function toBuffer(options) {
    ...
    // PLATFORM FIELDS SERIALIZATION DISABLED IN THIS FORK
    // if (this.version >= 2 && this.type === MASTERNODE_TYPE_HP) {
    //   payloadBufferWriter
  ...
  /**
   * Verify payload signature. Unlike the generic
   * `AbstractPayload.prototype.verifySignature`, this automatically uses
   * this chain's own `PublicKey` class to reconstruct the signer's key, so
   * callers don't need to pass one explicitly.
   * @param {string|Buffer} publicKeyId
   * @return {boolean}
   */
  ProRegTxPayload.prototype.verifySignature = function (publicKeyId) {
    return AbstractPayload.prototype.verifySignature.call(
      this,
      publicKeyId,
      PublicKey
    );
  ...
  byConfig.set(config, ProRegTxPayload);

  return ProRegTxPayload;
}

module.exports = { createProRegTxPayloadClass: createProRegTxPayloadClass };
