...
var transactionSignatureClassCache = new WeakMap();

/**
 * Creates a TransactionSignature class bound to the given `Networks`
 * instance (used to resolve the wrapped `PublicKey`'s network).
 *
 * @param {Object} Networks - a `lib/networks.js` `createNetworks()` instance
 * @returns {TransactionSignature}
 */
function createTransactionSignatureClass(Networks) {
  if (transactionSignatureClassCache.has(Networks)) {
    return transactionSignatureClassCache.get(Networks);
  }
  ...
  transactionSignatureClassCache.set(Networks, TransactionSignature);

  return TransactionSignature;
}

module.exports = {
  createTransactionSignatureClass: createTransactionSignatureClass,
};
