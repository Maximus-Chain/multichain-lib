...
var registeredTransactionTypes = require('../constants').registeredTransactionTypes;
var WrongOutPointError = errors.WrongOutPointError;
var WrongPublicKeyHashError = errors.WrongPublicKeyHashError;

var transactionClassCache = new WeakMap();

/**
 * Creates a Transaction class bound to the given `Networks` instance.
 * `sha256sha256`/`sha256ripemd160` (used to compute the transaction hash and
 * the pubkey hash for signing) are pure, stateless functions shared across
 * all chains — `Networks` is only needed to obtain this chain's bound
 * `Address`/`Script`/`Input`/`Output`/`UnspentOutput`/`PrivateKey`/`Sighash`/
 * `Payload` classes.
 *
 * @param {Object} Networks - a `lib/networks.js` `createNetworks()` instance
 * @param {Object} [config] - the chain's `ChainConfig`. Forwarded to the
 *   Payload factory so the chain's `ProRegTxPayload` class (with its
 *   per-chain defaults and validation rules) is the same instance the
 *   factory exposes as `mc.ProRegTxPayload`. Without it, `setExtraPayload`
 *   would `instanceof`-check against a different class and reject the
 *   payload.
 * @returns {Transaction}
 */
function createTransactionClass(Networks, config) {
  if (transactionClassCache.has(Networks)) {
    return transactionClassCache.get(Networks);
  }

  var Address = require('../address').createAddressClass(Networks);
  var Script = require('../script/script').createScriptClass(Networks);
  Script.Interpreter = require('../script/interpreter').createInterpreterClass(
    Networks
  );
  var PrivateKey = require('../privatekey').createPrivateKeyClass(Networks);

  var Output = require('./output').createOutputClass(Networks);
  var UnspentOutput =
    require('./unspentoutput').createUnspentOutputClass(Networks);
  ...
  var Payload = require('./payload').createPayloadClass(Networks, config);
  ...
  transactionClassCache.set(Networks, Transaction);

  return Transaction;
}

module.exports = { createTransactionClass: createTransactionClass };
