...
const scriptModule = require('../script/script');
const addressModule = require('../address');

const unspentOutputClassCache = new WeakMap();

/**
 * Creates an UnspentOutput class bound to the given `Networks` instance
 * (used for its `Address` and `Script`).
 *
 * @param {Object} Networks - a `lib/networks.js` `createNetworks()` instance
 * @returns {UnspentOutput}
 */
function createUnspentOutputClass(Networks) {
  if (unspentOutputClassCache.has(Networks)) {
    return unspentOutputClassCache.get(Networks);
  }

  const Script = scriptModule.createScriptClass(Networks);
  const Address = addressModule.createAddressClass(Networks);
  ...
  unspentOutputClassCache.set(Networks, UnspentOutput);

  return UnspentOutput;
}

module.exports = { createUnspentOutputClass };
